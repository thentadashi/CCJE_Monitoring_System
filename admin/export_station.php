...
$station = count($mysqli->query($sql)->fetch_all(MYSQLI_ASSOC));
...
$pdf->SetFont('helvetica', 'B', 10);
$pdf->Cell(0, 10, 'Station List', 0, 1, 'C');
...
$pdf->Cell(20, 10, 'Station', 1, 0, 'C',true);
$pdf->Cell(45, 10, 'Name', 1, 0, 'C',true);
$pdf->Cell(25, 10, 'Assign Date', 1, 0, 'C',true);
$pdf->Cell(25, 10, 'Contact', 1, 0, 'C',true);
$pdf->Cell(65, 10, 'Address', 1, 1, 'C',true);
...
while ($row = $result->fetch_assoc()) {
    ...
    $pdf->Cell(20, 7, $row['sti_station'], 1, 0, 'L');
    $pdf->Cell(45, 7, $row['sti_lname'] . ", " . $row['sti_fname'] . " " . $row['sti_mname'] . " " . $row['sti_sname'], 1, 0, 'L');
    $pdf->Cell(25, 7, date( "M, d, Y" ,strtotime($row['sti_assign_date'])), 1, 0, 'L');
    $pdf->Cell(25, 7, $row['sti_contact'], 1, 0, 'L');
    $pdf->Cell(65, 7, ucwords($row['sti_barangay']) . ", " . ucwords(strtolower($row['sti_municipal'])) . ", " . ucwords(strtolower($row['sti_region'])), 1, 1, 'L');
}
