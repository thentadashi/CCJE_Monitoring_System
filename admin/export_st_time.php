...
?>
   ...
   if ($block === "all") {
    $sql = "SELECT * FROM student_enroll JOIN student_time on student_enroll.id=student_time.std_id WHERE student_enroll.status !='1' ORDER BY student_enroll.last_name ASC";
    $result = $mysqli->query($sql);
   }else{
    $sql = "SELECT * FROM student_enroll JOIN student_time on student_enroll.id=student_time.std_id WHERE student_enroll.status !='1' and student_enroll.block='$block' ORDER BY student_enroll.last_name ASC";
    $result = $mysqli->query($sql);
   }
   ...
   $pdf->SetFont('helvetica', 'B', 10);
   $pdf->Cell(0, 10, 'Student Time Sheet', 0, 1, 'C');
   ...
   $pdf->Cell(35, 10, 'Spend', 1, 1, 'C',true);
   ...
   while ($row = $result->fetch_assoc()) {
       ...
       $pdf->Cell(20, 7, $std, 1, 0, 'L');
       $pdf->Cell(70, 7, $row['last_name'] . ", " . $row['first_name'] . " " . $row['middle_name'] . " " . $row['suffix_name'], 1, 0, 'L');
       $pdf->Cell(20, 7, $row['block'], 1, 0, 'C');
       
        $sqltime = "SELECT * FROM attendance_qrcode WHERE std_id= '$std' AND date_time BETWEEN '$dateget 00:00:00' AND '$dateget 23:59:59' ;";
             ...
             if ($resulttime->num_rows > 0) {
                ...
                while ($time = $resulttime->fetch_assoc()) {
                    $timeValue += strtotime($time['spend']);
                        ...
                        $pdf->Cell(35, 7, sprintf("%02d", $hours) . ":" . sprintf("%02d", $minutes) . ":" . sprintf("%02d", $seconds)." hours ", 1, 1, 'C');
                       
                                                      
            } else {
                        $pdf->Cell(35, 7, "0 hours ", 1, 1, 'C');
